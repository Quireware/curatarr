...
impl Repository for SqliteRepository {
    ...
    async fn create_author(&self, author: &NewAuthor) -> Result<Author, DbError> {
        ...
        for ext in &author.external_ids {
            metadata_repo::upsert_external_id(&self.pool, EntityKind::Author, &id.to_string(), ext)
                .await?;
        }
    ...
    async fn get_author(&self, id: AuthorId) -> Result<Option<Author>, DbError> {
        ...
        match row.as_ref() {
            None => Ok(None),
            Some(row) => {
                let mut author = author_from_row(row);
                author.external_ids = metadata_repo::list_external_ids(
                    &self.pool,
                    EntityKind::Author,
                    &author.id.to_string(),
                )
    ...
    async fn create_series(&self, series: &NewSeries) -> Result<Series, DbError> {
        ...
        for ext in &series.external_ids {
            metadata_repo::upsert_external_id(&self.pool, EntityKind::Series, &id.to_string(), ext)
                .await?;
        }
    ...
    async fn get_series(&self, id: SeriesId) -> Result<Option<Series>, DbError> {
        ...
        match row.as_ref() {
            ...
            Some(row) => {
                ...
                series.external_ids = metadata_repo::list_external_ids(
                    &self.pool,
                    EntityKind::Series,
                    &series.id.to_string(),
                )
    ...
    async fn list_external_ids(
        ...
        metadata_repo::list_external_ids(&self.pool, kind, entity_id).await
    }
    ...
    async fn upsert_external_id(
        ...
        metadata_repo::upsert_external_id(&self.pool, kind, entity_id, id).await
    }

    async fn list_field_locks(
        &self,
        kind: EntityKind,
        entity_id: &str,
    ) -> Result<Vec<FieldLock>, DbError> {
        metadata_repo::list_field_locks(&self.pool, kind, entity_id).await
    }

    async fn set_field_lock(
        &self,
        kind: EntityKind,
        entity_id: &str,
        field: &str,
    ) -> Result<FieldLock, DbError> {
        metadata_repo::set_field_lock(&self.pool, kind, entity_id, field).await
    }

    async fn clear_field_lock(
        &self,
        kind: EntityKind,
        entity_id: &str,
        field: &str,
    ) -> Result<(), DbError> {
        metadata_repo::clear_field_lock(&self.pool, kind, entity_id, field).await
    }

    async fn clear_all_field_locks(
        &self,
        kind: EntityKind,
        entity_id: &str,
    ) -> Result<(), DbError> {
        metadata_repo::clear_all_field_locks(&self.pool, kind, entity_id).await
    }

    async fn list_field_sources(
        &self,
        kind: EntityKind,
        entity_id: &str,
    ) -> Result<Vec<FieldSource>, DbError> {
        metadata_repo::list_field_sources(&self.pool, kind, entity_id).await
    }

    async fn upsert_field_source(
        &self,
        kind: EntityKind,
        entity_id: &str,
        field: &str,
        ...
        metadata_repo::upsert_field_source(&self.pool, kind, entity_id, field, source).await
    }

    async fn append_audit(&self, event: &NewAuditEvent) -> Result<AuditEvent, DbError> {
        metadata_repo::append_audit(&self.pool, event).await
    }
    ...
    async fn list_audit(
        ...
        metadata_repo::list_audit(&self.pool, kind, entity_id, page).await
    }

    async fn find_author_by_name(&self, name: &str) -> Result<Option<Author>, DbError> {
        let Some(id) = metadata_repo::find_author_id(&self.pool, name).await? else {
            return Ok(None);
        };
        self.get_author(AuthorId::from_uuid(parse_uuid(&id))).await
    }

    async fn find_series_by_title(&self, title: &str) -> Result<Option<Series>, DbError> {
        let Some(id) = metadata_repo::find_series_id(&self.pool, title).await? else {
            return Ok(None);
        };
        self.get_series(SeriesId::from_uuid(parse_uuid(&id))).await
    }

    async fn find_tag_by_name(&self, name: &str) -> Result<Option<Tag>, DbError> {
        let Some(id) = metadata_repo::find_tag_id(&self.pool, name).await? else {
            return Ok(None);
        };

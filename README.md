...
Phase 1 (foundation, scan and import) and Phase 2 (metadata enrichment) are complete:
...
- Metadata providers (OpenLibrary, Google Books, ComicVine, AniList, MangaDex, MangaUpdates, Hardcover, ISBNdb, MyAnimeList, LibraryThing; Goodreads disabled), field merge/locks/audit, refresh API
...
| POST | `/api/v1/works/{id}/refresh-metadata?preview=true` | merge from configured providers |
| POST | `/api/v1/works/{id}/apply-metadata` | body: `{"provider","external_id"}` |
| GET/PUT/POST | `/api/v1/works/{id}/field-locks` | lock fields against provider overwrites |
| GET | `/api/v1/works/{id}/metadata-audit`, `/metadata-sources` | |
| POST | `/api/v1/works/refresh-metadata` | bulk; body `{content_type?, preview?}` |
| POST | `/api/v1/metadata/search` | manual provider search |
| GET | `/api/v1/system/providers` | health, success rate, latency |
| POST | `/api/v1/authors/{id}/import-works` | search providers for that author |
| POST | `/api/v1/series/{id}/import-works` | search providers for that series |
